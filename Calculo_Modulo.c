...
int main()
{
	...
	printf("El modulo del vector es: %.3f", modulo);
	return 0;
}
...
float calculo_modulo(int valor_x, int valor_y, int valor_z)
{
	...
	if ((valor_x != 0) && (valor_y == 0) && (valor_z == 0))
	{
		printf("Es un versor del EJE X\n");
	}
	if ((valor_x == 0) && (valor_y != 0) && (valor_z == 0))
	{
		printf("Es un versor del EJE Y\n");
	}
	if ((valor_x == 0) && (valor_y == 0) && (valor_z != 0))
	{
		printf("Es un versor del EJE Z\n");
	}
	x2 = pow(valor_x, 2);
	y2 = pow(valor_y, 2);
	z2 = pow(valor_z, 2);
